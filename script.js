...
window.onload = () => {
    ...
    if (monthInput) {
        ...
        // ★修正ポイント：年月が変更されたら表を再生成するイベントを追加
...
function autoFillShift() {
    ...
    for (let s = 0; s < staffs.length; s++) {
        ...
        for (let d = 0; d < daysInMonth; d++) {
            if (grid[s][d] === "出勤" && !canWork(grid[s], d, limit)) grid[s][d] = "公休";
        }
    }
    ...
    for (let d = 0; d < daysInMonth; d++) {
        ...
        while (workers.length > dailyTargets[d]) {
    ...
    for (let s = 0; s < staffs.length; s++) {
        ...
        if (staffs[s].type === 'full') {
            ...
            while (currentOffs > 9 && adjustableIdxs.length > 0) {
                ...
                if (canWork(row, idx, 4)) { row[idx] = "出勤"; currentOffs--; }
...
function generateTable() {
    ...
    let fRow = '<td>目標人数</td>';
    for (let d = 1; d <= daysInMonth; d++) { fRow += `<td><input type="number" class="need-count-input" data-day="${d}" value="${defaultNeed}" style="width:30px"></td>`; }
...
function updateSummary() {
    ...
    summaryList.innerHTML = staffs.map((s, idx) => {
        ...
        return `<div class="summary-row"><strong>${s.name}</strong>: 出勤 ${counts["出勤"]}日 / 休み ${offSum}日</div>`;
    }).join('');
}
