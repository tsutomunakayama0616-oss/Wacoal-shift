...
function autoFillShift() {
    ...
    for (let d = 0; d < daysInMonth; d++) {
        ...
        candidates.sort((a, b) => (grid[a].filter(v=>v==="出勤").length - grid[b].filter(v=>v==="出勤").length) + (Math.random()-0.5));

        let assigned = grid.filter(row => row[d] === "出勤" || row[d] === "有給").length;
        ...
        // 3人目標の時だけ2人を許容。0,4,5人は絶対。
        let minNeeded = (target === 3) ? 2 : target;
        if (assigned < minNeeded) {
            alert(`【エラー】${d + 1}日はスタッフを確保できません。\n現在：${assigned}名（目標：${target}名）\n\n5連勤制限または希望休の重複が原因です。`);
            return;
        }
    }
    ...
    grid.forEach((row, sIdx) => {
        ...
        for (let i = 0; i < daysInMonth; i++) {
            if (row[i] === "") {
                let s = 0;
                for (let j = i-1; j>=0; j--) { if(row[j]==="出勤") s++; else break; }
                if (s < 4 && row.filter(v=>v==="出勤").length < (isFull ? 31 : 10)) row[i] = "出勤";
                else row[i] = "公休";
            }
        }
    ...
    updateSummary();
    alert("生成完了！「出勤」の5連勤を防止し、有給を反映しました。");
}
